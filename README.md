...
>
> **Зачем нужен канал?** Telegram file_id становятся недействительными со временем.
> Канал используется как долговременное хранилище для фото/видео упражнений и тикетов.
> При отправке медиа бот сначала пытается использовать file_id, а если не получается —
> пересылает из канала и обновляет file_id в базе данных.
