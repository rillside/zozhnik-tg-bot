...
def sleep_dashboard_keyboard(is_sleeping=False):
    keyboard = types.InlineKeyboardMarkup()
    if is_sleeping:
...
def sleep_cancel_custom_keyboard():
    keyboard = types.InlineKeyboardMarkup()
...
def xp_profile_keyboard():
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton("⬅️ Статистика", callback_data='back_to_stats'))
    return keyboard


def user_profile_admin_keyboard(user_id: int, status: str):
    """Клавиатура для просмотра и управления профилем пользователя."""
    keyboard = types.InlineKeyboardMarkup()
    if status == 'BANNED':
