...
def adm_start_message(first_name: str, id: int) -> str:
    """Приветственное сообщение для входа администратора/владельца в панель управления."""
    adm_start_msg = f"""
...
def success_new_admin(name: str) -> str:
    """Уведомление об успешном назначении администратора."""
    return f"✅ Пользователь {name.strip()} теперь администратор!"
def success_remove_admin(name: str) -> str:
    """Уведомление об успешном снятии прав администратора."""
...
def water_goal_success_msg(volume: int) -> str:
    """Уведомление об успешной установке цели по воде."""
...
def activity_goal_success_msg(goal: int) -> str:
    """Уведомление об успешной установке цели по активности."""
...
def profile_xp_msg(first_name: str, xp: int, level: int, rank: int | None) -> str:
    ...
    to_next = xp_for_next_level(xp, xp_per_level=xp_per_level)
...
admin_access_denied_msg = "⛔ Нет доступа."
